...
pub async fn get_app_config(
    ...
    let row = sqlx::query!(
        "SELECT mod_log_channel_id, response_channel_id, accepted_role_id FROM guild_config WHERE guild_id = ?",
        guild_id_str
    )
    .fetch_optional(pool)
    .await?;
...
pub async fn set_app_config(
    ...
    sqlx::query!(
        "INSERT INTO guild_config (guild_id, mod_log_channel_id, response_channel_id, accepted_role_id) VALUES (?, ?, ?, ?)
